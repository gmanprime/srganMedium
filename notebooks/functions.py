...
def timecheck(train, val, test):
    ...
    """
    train_timecheck = np.arange(
        np.datetime64("2000-01-01T06"),
        np.datetime64("2014-01-01T12"),
        np.timedelta64(6, "h"),
    )
    val_timecheck = np.arange(
        np.datetime64("2014-01-01T12"),
        np.datetime64("2016-12-31T18"),
        np.timedelta64(6, "h"),
    )
    test_timecheck = np.arange(
        np.datetime64("2016-12-31T18"),
        np.datetime64("2020-01-01T00"),
        np.timedelta64(6, "h"),
    )
    print(
        "train:", set(train_timecheck) - set(train.time.values.astype("datetime64[h]"))
    )
...
def resampling(partial_dataset, var):
    """
    Resample the dataset to 6-hourly interval and interpolate missing values

    Args:
        partial_dataset (xr.Dataset): Partial dataset
        var (str): Variable to be resampled

    Returns:
        partial_dataset (xr.Dataset): Resampled dataset
    """
...
def transform_train(trainds, var):
    """
    Transform the training dataset using MinMaxScaler and log10 transformation for precipitation variable (apcp)

    Args:
        trainds (xr.Dataset): Training dataset,
        var (str): Variable to be transformed

    Returns:
        scaler_train (MinMaxScaler): Scaler used for training dataset
        train
    """
    if var == "apcp":  # tp
        scaler_train = MinMaxScaler()
        x_one_col = trainds.tp.values.reshape(
            [
                trainds.tp.values.shape[0]
                * trainds.tp.values.shape[1]
                * trainds.tp.values.shape[2],
                1,
            ]
        )
        x_one_col = np.log10(x_one_col + 1)  # 10**X_one_col - 1 to scale back
        x_one_col_res = scaler_train.fit_transform(
            x_one_col
        )  # scaler_train.inverse_transform(X_one_col_res) to scale back, or use 10**scaler_train.inverse_transform(X_one_col_res) -1 only
        trainds.tp.values = x_one_col_res.reshape(trainds.tp.values.shape)
        return scaler_train, trainds

    if var == "RAINNC":  # tp
        scaler_train = MinMaxScaler()
        x_one_col = trainds.RAINNC.values.reshape(
            [
                trainds.RAINNC.values.shape[0]
                * trainds.RAINNC.values.shape[1]
                * trainds.RAINNC.values.shape[2],
                1,
            ]
        )
        x_one_col = np.log10(x_one_col + 1)  # 10**X_one_col - 1 to scale back
    ...
    if var == "pwat":
        scaler_train = MinMaxScaler()
        x_one_col = trainds.pwat.values.reshape(
            [
                trainds.pwat.values.shape[0]
                * trainds.pwat.values.shape[1]
                * trainds.pwat.values.shape[2],
                1,
            ]
        )
        # X_one_col = np.log10(X_one_col+1) # 10**X_one_col - 1 to scale back
    ...
    if var == "cape":
        scaler_train = MinMaxScaler()
        x_one_col = trainds.cape.values.reshape(
            [
                trainds.cape.values.shape[0]
                * trainds.cape.values.shape[1]
                * trainds.cape.values.shape[2],
                1,
            ]
        )
        # X_one_col = np.log10(X_one_col+1) # 10**X_one_col - 1 to scale back
    ...
    if var == "cin":
        scaler_train = MinMaxScaler()
        x_one_col = trainds.cin.values.reshape(
            [
                trainds.cin.values.shape[0]
                * trainds.cin.values.shape[1]
                * trainds.cin.values.shape[2],
                1,
            ]
        )
        # X_one_col = np.log10(X_one_col+1) # 10**X_one_col - 1 to scale back
    ...
    if var == "t2m":
        scaler_train = MinMaxScaler()
        x_one_col = trainds.t2m.values.reshape(
            [
                trainds.t2m.values.shape[0]
                * trainds.t2m.values.shape[1]
                * trainds.t2m.values.shape[2],
                1,
            ]
        )
        # X_one_col = np.log10(X_one_col+1) # 10**X_one_col - 1 to scale back
...
def transform_val_test(val_test, scaler_train, is_prec=True):
    ...
    if is_prec:
        x_one_col = val_test.values.reshape(
            [
                val_test.values.shape[0]
                * val_test.values.shape[1]
                * val_test.values.shape[2],
                1,
            ]
        )
...
def inverse_val_test(transformed_vt, scaler_train, is_prec=True):
    ...
    """
    if is_prec == True:
        X_one_col = transformed_vt.values.reshape(
            [
                transformed_vt.values.shape[0]
                * transformed_vt.values.shape[1]
                * transformed_vt.values.shape[2],
                1,
            ]
        )
